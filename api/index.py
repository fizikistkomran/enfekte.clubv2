...
# -----------------------------------------------------------
# Flask app (Vercel dizinleri: api/ altında çalışıyoruz)
# templates: proje-kökü/templates, static: proje-kökü/static
# -----------------------------------------------------------
HERE = os.path.dirname(__file__)
TEMPLATES_DIR = os.path.normpath(os.path.join(HERE, "..", "templates"))
STATIC_DIR = os.path.normpath(os.path.join(HERE, "..", "static"))

app = Flask(__name__, template_folder=TEMPLATES_DIR, static_folder=STATIC_DIR)
app.secret_key = os.getenv('SECRET_KEY', 'super-secret-key')  # prod'da ENV ile ver
...
def get_db_connection():
    ...
    if os.getenv("DATABASE_URL"):
        ...
        # Geri uyumluluk için yüklediğin değerler
        conn_kwargs = dict(
            dbname="railway",
            user="postgres",
...
def home():
    ...
    # home.html yoksa index.html'i kullan
    tpl = "home.html" if _template_exists("home.html") else "index.html"
    return render_template(tpl, nodes=nodes, edges=edges)
...
def create():
    ...
    # create.html yoksa index.html'i göster
    return render_template("create.html" if _template_exists("create.html") else "index.html")
...
def login():
    ...
    # login.html yoksa index.html'i göster
    return render_template("login.html" if _template_exists("login.html") else "index.html")
...
def user_page(slug):
    ...
    # user_page.html yoksa dashboard.html ya da index.html'e düş
    for candidate in ["user_page.html", "dashboard.html", "index.html"]:
        if _template_exists(candidate):
            return render_template(candidate, nodes=nodes_vis, edges=edges_vis, slug=slug, is_owner=is_owner)
...
def edit_page(slug):
    ...
    # edit.html yoksa club_dashboard.html ya da index.html'e düş
    for candidate in ["edit.html", "club_dashboard.html", "index.html"]:
        if _template_exists(candidate):
            return render_template(candidate, slug=slug, name=owner_name, connections=connections, nodes=nodes_vis, edges=edges_vis)
    return "Şablon bulunamadı"
...
# Vercel: modül seviyesinde 'app' objesi yeterli (WSGI)
# local çalıştırmak istersen:
